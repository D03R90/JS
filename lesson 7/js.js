...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    this.usedCells.push(foodCell); //Ошибка была тут.
  },
};
 ...
 * @property {int} maxX Максимальная позиция змейка на карте по оси X.
 * @property {int} maxY Максимальная позиция змейка на карте по оси Y.
 */
const snake = {
  body: null,
  direction: null,
   ...
   * @property {int} maxX Максимальная позиция змейка по оси X.
   * @property {int} maxY Максимальная позиция змейка по оси Y.
   */
 ...
 */
  ...
  init(settings = {}) {
    ...
    this.countEl = document.querySelector(this.settings.showScoreSelector);
    this.drop();
  },
...
const game = {
  ...
  //     headCell[1] = 0;
  //   table = document.getElementById(headCell.join());
  // } 
  /**
